...
switch ($action) {
    case 'init':
        // Fetch User Info
        $stmt = $conn->prepare("SELECT user_id, username, profile_img FROM users WHERE user_id = ?");
        $stmt->bind_param("i", $user_id);
        $stmt->execute();
        $me = $stmt->get_result()->fetch_assoc();
        $me['profile_img'] = $me['profile_img'] ?? '/imgandgifs/login.png';
        ...
        while ($row = $friend_res->fetch_assoc()) {
            ...
            $my_friends[] = [
                ...
                'profile_img' => $row['profile_img'] ?? '/imgandgifs/login.png',
                'status' => $row['status'],
        ...
        $res = $conn->query("SELECT user_id, username, profile_img FROM users WHERE user_id NOT IN ($exclude_ids) LIMIT 50");
    ...
    case 'check_updates':
        $last_id = intval($_GET['last_id'] ?? 0);
        // Fetch new messages for ME from ANY sender
        // Fetch new messages for ME (received) OR from ME (sent elsewhere)
        $sql = "SELECT m.id, m.sender_id, m.receiver_id, m.message, m.media, m.created_at, u.username, u.profile_img 
                FROM messages m
        ...
        while ($row = $res->fetch_assoc()) {
            $updates[] = [
                ...
                'sender_img' => $row['profile_img'] ?? '/imgandgifs/login.png',
                'message' => htmlspecialchars($row['message']),
                'media' => $row['media'],
                'time' => date('H:i', strtotime($row['created_at']))
            ];
    ...
    case 'send_message':
        ...
        if (isset($_FILES['media']) && $_FILES['media']['size'] > 0) {
            ...
            // Basic validation
            $allowed = ['jpg', 'jpeg', 'png', 'gif', 'mp4', 'webm', 'webp', 'jfif'];
